        ...
        <div className="p-6 border-t border-gray-100 flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-4 text-sm text-gray-600">
            <div className="flex items-center gap-1 font-bold">
              <Clock size={16} /> {pickup.distance || "Calculating..."}
            </div>
            <div className="flex items-center gap-1 font-bold text-[#FF9800] uppercase tracking-wider">
              {pickup.scrapType}
            </div>
          </div>
...
const KabadBechoDriverDashboard = () => {
  ...
  const myPickups = useMemo(
    () =>
      (pickups || []).filter((p) => {
        ...
        // pending & accepted sabke liye
        if (p.status === "pending" || p.status === "accepted") return true;

        // completed bhi dikhao (driverId ignore karke)
  ...
  const myRoutes = useMemo(
    () => (assignedRoutes || []).filter((r) => r.driverId === currentDriverId),
  ...
  const driverStats = useMemo(
    () => ({
      ...
      pendingToday: myPickups.filter(
        (p) => p.status !== "completed" && p.status !== "denied",
      ).length,
      totalEarnings:
        "₹" +
        myPickups
          ...
          .reduce((acc, curr) => {
            const amt =
              parseInt(
                (curr.collectedAmount || curr.amount || "0")
                  .toString()
                  .replace(/[^\d]/g, ""),
              ) || 0;
            return acc + amt;
      ...
      const unsubOrders = onSnapshot(
        ...
        (snapshot) => {
          ...
          const needsGeocoding = orders.filter(
            (o) => o.address && !o.locationGeocoded && (!o.lat || !o.lng),
          );
  ...
  const handleCompletePickup = useCallback(
    async (pickupId) => {
      try {
        ...
        if (receiptImage) {
          imageUrl = await uploadToCloudinary(receiptImage);
        }
    ...
    [receiptImage, finalAmount, finalWeight],
  );
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        {[
          ...
          <button
            ...
            />
            <span
              className={`font-semibold ${activeTab === item.id ? "font-bold" : ""}`}>
              {item.label}
            </span>
            {activeTab === item.id && (
      ...
      <div className="p-4 border-t border-gray-100">
        <div className="bg-[#F1F8E9] rounded-xl p-4 flex items-center gap-3 mb-2">
          ...
          <div className="flex-1 overflow-hidden">
            <h4 className="font-bold text-[#2E7D32] text-sm truncate">
              {driverStats.name}
            </h4>
            <p className="text-xs text-[#558B2F] truncate">
              ID: {driverStats.id}
            </p>
          </div>
        </div>
  ...
  const MyRoutes = () => (
    <div className="p-8 animate-fadeIn pb-24 md:pb-8">
      ...
      <p className="text-gray-500 text-sm mb-6">
        Routes optimized by admin using Clarke-Wright algorithm. Follow the
        sequence numbers for optimal pickup order.
      </p>
      ...
      {myRoutes.length === 0 ? (
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <p className="text-gray-600">
            The admin hasn't assigned routes yet. Check back soon.
          </p>
        </div>
      ) : (
            ...
            <div
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-[#2E7D32]">
                        {route.shiftSlot?.toUpperCase()} SHIFT
                      </h3>
                  ...
                  <div className="text-right">
                    <span className="text-xs font-bold px-2 py-1 bg-blue-50 text-blue-700 rounded-md border border-blue-100">
                      {route.totalQuantity} kg
                    </span>
                    <p className="text-[10px] text-gray-400 mt-1">
                      {route.estimatedDistanceKm?.toFixed(1)} km est.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4">
                <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <Hash size={14} className="text-[#4CAF50]" /> Pickup Sequence
                  (Follow this order)
                </h4>
            ...
            {[
              {
                ...
                value: driverStats.todayPickups,
              ...
              {
                icon: <Clock size={24} />,
                value: driverStats.pendingToday,
                label: "Pending",
              },
              {
              ...
              <div
                key={idx}
                className="bg-white/20 backdrop-blur-sm p-4 rounded-xl border border-white/10 shadow-sm hover:bg-white/30 transition-colors">
            ...
            <div className="mt-8 bg-white/10 backdrop-blur-md p-2 rounded-2xl border border-white/20">
              <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 mb-2">
                <h3 className="font-bold text-sm tracking-wide">
                  YOUR OPTIMIZED SHIFT ROUTE
                </h3>
                <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded-full uppercase">
                  {myRoutes[0]?.shiftSlot} Shift
                </span>
              </div>
              <div className="h-[250px] rounded-xl overflow-hidden shadow-inner">
                <LiveMap
                  routeStops={(myRoutes[0]?.stops || []).filter(
                    (s) => s.lat && s.lng,
                  )}
                  complexRoute={myRoutes[0]?.polyline || null}
                />
              </div>
              <p className="text-[10px] text-center mt-2 opacity-80 italic">
                Route synchronized with the Admin's latest optimization.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-3 bg-white p-2 rounded-2xl shadow-sm inline-flex min-w-max">
          {[
            ...
            },
            {
              id: "completed",
              label: "Completed",
              count: myPickups.filter((p) => p.status === "completed").length,
            },
          ].map((tab) => (
            <button
              key={tab.id}
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        {pickup.receipt && (
                          <div className="mt-3">
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="mt-2 flex items-center justify-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-lg border border-blue-100 hover:bg-blue-100 transition text-sm font-semibold">
                              ⬇ Download Receipt
                            </a>
                          </div>
                        )}
                      ...
                      <div className="bg-[#F8FAF8] p-4 rounded-xl border border-gray-100">
                        <div className="flex items-center space-x-3 mb-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-[#66BB6A] to-[#4CAF50] rounded-full flex items-center justify-center text-white font-bold">
                        ...
                        <div className="flex items-start space-x-2 text-sm text-gray-700">
                          <MapPin
                            className="flex-shrink-0 mt-0.5 text-[#66BB6A]"
                            size={16}
                          />
                          <div>
                            <p className="font-medium">
                              {pickup.address || "Address not set"}
                            </p>
                            {pickup.landmark && (
                        ...
                        <>
                          {pickup.lat && pickup.lng && (
                            <button
                              ...
                              className="flex items-center justify-center space-x-2 px-4 py-3 bg-gradient-to-r from-[#66BB6A] to-[#4CAF50] text-white font-semibold rounded-xl transition transform hover:-translate-y-1">
                              <Navigation size={18} />
                              <span>Navigate</span>
                            </button>
                          )}
                          <button
                            ...
                            className="flex items-center justify-center space-x-2 px-4 py-3 bg-white border-2 border-[#66BB6A] text-[#66BB6A] font-semibold rounded-xl hover:bg-[#E8F5E9] transition">
                            <CheckCircle size={18} />
                            <span>Complete</span>
                          </button>
                        </>
            ...
            <div
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="flex items-center justify-center md:justify-start gap-4 mt-2 text-sm text-gray-500">
                  ...
                  <span className="flex items-center gap-1">
                    <Clock size={14} /> {pickup.completedAt}
                  </span>
                </div>
              </div>
    ...
    <div className="p-8 max-w-4xl mx-auto animate-fadeIn pb-24 md:pb-8">
      ...
      <div className="bg-white rounded-3xl shadow-lg p-8 border border-gray-100">
        <div className="flex flex-col md:flex-row items-center gap-8 mb-8 border-b pb-8">
          <div className="w-32 h-32 bg-[#E8F5E9] rounded-full flex items-center justify-center text-5xl font-bold text-[#2E7D32] border-4 border-white shadow-xl">
            {driverStats.name.charAt(0)}
          </div>
          <div className="text-center md:text-left">
            <h3 className="text-3xl font-bold text-[#5D4037]">
              {driverStats.name}
            </h3>
            <p className="text-gray-500 text-lg">
              Partner ID: {driverStats.id}
            </p>
            <div className="mt-4 flex gap-4">
              <div className="px-4 py-2 bg-blue-50 rounded-lg text-blue-700 font-bold border border-blue-100">
                {driverStats.vehicleNo}
              </div>
              <div className="px-4 py-2 bg-yellow-50 rounded-lg text-yellow-700 font-bold border border-yellow-100">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-xl w-full p-8 animate-fadeIn">
            ...
            <h2 className="text-2xl font-bold text-[#5D4037] mb-6 flex items-center gap-2">
              ...
              Complete Pickup {selectedPickup.id}
            </h2>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-4 border rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4CAF50]"
                />
              </div>
              ...
              <div>
                ...
                <label className="border-2 border-dashed p-6 rounded-xl text-center text-gray-400 cursor-pointer block hover:border-[#4CAF50] transition">
                  <Camera className="mx-auto mb-2" />
                  <span className="text-sm">
                    {receiptImage ? "Change Image" : "Click to upload"}
                  </span>
                ...
                {receiptImage && (
                  <img
                    ...
                    alt="preview"
                    className="mt-4 h-32 rounded-lg object-cover mx-auto"
                  />
                )}
              </div>
            </div>
            ...
            <div className="flex gap-4 mt-8">
              <button
                ...
                className="flex-1 p-4 bg-gray-100 rounded-xl font-bold hover:bg-gray-200 transition">
                Cancel
              </button>
              ...
              <button
                ...
                disabled={uploading}
                className="flex-1 p-4 bg-[#4CAF50] text-white rounded-xl font-bold hover:bg-[#43A047] transition disabled:opacity-50">
