...
const BookPickup = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        "Pickup scheduled successfully! You can track it in your dashboard.",
      );
      onClose();
    } catch (err) {
              ...
              <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <div className="space-y-6">
                  <h3 className="text-xl font-bold text-[#5D4037] flex items-center gap-3">
                    <span className="w-7 h-7 rounded-full bg-[#66BB6A] text-white flex items-center justify-center text-xs font-bold">
                      1
                    </span>
                    Contact Details
                  </h3>
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      rows="2"
                      className="md:col-span-2 w-full px-5 py-3 rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-[#66BB6A] text-sm"
                      required
                    />
                  </div>
                </div>
                    ...
                    {scrapTypes.map((type) => {
                      ...
                      return (
                        <button
                          ...
                          <div
                            className={`absolute top-2 right-2 w-5 h-5 rounded-full flex items-center justify-center ${isSelected ? "bg-[#66BB6A]" : "bg-gray-200"}`}>
                            {isSelected && (
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <input
                      ...
                      name="date"
                      onChange={handleInputChange}
                      className="w-full px-5 py-3 rounded-xl bg-gray-50 border border-gray-200 outline-none"
                      required
                    />
                    <select
                      name="time"
                ...
                <button
                  type="submit"
                  className="w-full py-4 bg-[#66BB6A] text-white rounded-xl font-bold text-lg hover:bg-[#4CAF50] transition-all flex items-center justify-center gap-2 shadow-lg shadow-green-200">
